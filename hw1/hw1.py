...
def test_conv2d(mode='zero'): 
  ...
  # for l in range((filt_l // 2, length + filt_l // 2)): 
  #   for w in range((filt_w // 2, width + filt_w // 2)):
  #     crop = image[l - filt_l // 2: l + filt_l // 2, w - filt_w // 2, w + filt_w // 2]
  #     crop = image[l:(l + 2*filt_l), w:(w + 2*filt_w)]
  #     result[l - filt_l // 2, w - filt_w //2 ] = sum(np.multiply(np.array(crop), np.array(filt))) 
